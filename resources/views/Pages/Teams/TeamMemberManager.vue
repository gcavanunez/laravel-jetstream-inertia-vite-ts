...
export default defineComponent({
  ...
  setup(props) {
    ...
    const { value } = usePage<MyPage>().props;
    ...
    const leaveTeam = () => {
      leaveTeamForm.delete(
        route("team-members.destroy", [props.team, value.props.user])
      );
    };
    ...
    const displayableRole = (role: string) => {
      return props.availableRoles.find((r: any) => r.key === role).name;
    };
